...
def iniciaSesion():
    ...
    if(numerousuarios == 0):
        ...
        if existe == True:
            ...
            etiquetaicono.pack()
            botonnuevanota = ttk.Button(marco2, text="Nueva nota", command=lambda: creaNota('', 'white'))
...
def guardaNotas():
    for i in notas:
        ...
        # Verificamos si una nota con el mismo texto ya existe en la base de datos
        cursor.execute("SELECT * FROM notas WHERE texto = ?", (i.texto,))
        datos = cursor.fetchall()
        if datos:
            existe = True
...
def creaNota(textoinicio,colorinicio):
    global notas
    global identificador
    fecha = str(int(time.time()))
    
    notas.append(Nota(identificador,textoinicio,colorinicio,fecha))
