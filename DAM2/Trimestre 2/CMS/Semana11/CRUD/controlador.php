...
class Supercontrolador
{
    ...
    function leer($tabla)
    {
        ...
        while ($fila = $resultado->fetch_assoc()) {
            ...
            if ($comment === null) {
                echo 'Error decoding JSON for field ' . $fila["Field"] . ': ' . json_last_error_msg() . ' - ' . $fila["Comment"];            } else {
        ...
        while ($fila = $resultado->fetch_assoc()) {
            ...
            foreach ($fila as $nombre_campo => $valor) {
                ...
                if (isset($comentario['FKtabla']) && $comentario['FKtabla'] != "") {
                    // Consulta preparada para obtener el valor relacionado
                    $consulta2 = $mysqli->prepare("SELECT " . $comentario['FKmostrar'] . " AS campo FROM " . $comentario['FKtabla'] . " WHERE " . $comentario['FKclave'] . " = ?");
                    $consulta2->bind_param("s", $valor);

                    echo 'Consulta SQL: ' . $consulta2->sqlstate . ' - ' . $consulta2->sqlstate;
                    ...
                    if ($fila2 = $resultado2->fetch_assoc()) {
                        ...
                        // Opción de depuración: Imprimir consulta y comentario
                        echo $consulta2;
                        echo '>'.$consulta2->error.'</td>';
                    }
            ...
            // Imprimir las opciones de actualizar y eliminar
            echo '<td>';
