...
class AventuraChico(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("AventuraChico")
        self.state('zoomed')
        style = ttk.Style()
...
##        #pygame.mixer.music.play()
        ...
        img = Image.open("./img/chicoelec.png")
        img = img.resize((self.winfo_screenwidth(), self.winfo_screenheight()), Image.LANCZOS)
        bg = ImageTk.PhotoImage(img)
        ...
        img = Image.open("./img/chicomalaeleccion.png")
        img = img.resize((self.winfo_screenwidth(), self.winfo_screenheight()), Image.LANCZOS)
        bg = ImageTk.PhotoImage(img)
        ...
        img = Image.open("./img/INTROFONDO.jpg")
        img = img.resize((self.winfo_screenwidth(), self.winfo_screenheight()), Image.LANCZOS)
        bg = ImageTk.PhotoImage(img)
        ...
        self.btn_eleccion1 = ttk.Button(self.b3, text="Unirse a los agresores y participar en el acoso en linea", style="Custom2.TButton", command=self.eleccion_mala)
        self.btn_eleccion1.place(relx=0.5, rely=0.9, anchor=tk.CENTER)

        self.btn_eleccion2 = ttk.Button(self.b3, text="Hablar con la compañera y ofrecerle su spoyo, amistad y consuelo", style="Custom2.TButton", command=self.eleccion_buena)
        self.btn_eleccion2.place(relx=0.5, rely=0.7, anchor=tk.CENTER)

        self.btn_eleccion3 = ttk.Button(self.b3, text="Hablar con sus padres para que informen a la escuela del comportamiento de ese chico", style="Custom2.TButton", command=self.eleccion_buena)
        self.btn_eleccion3.place(relx=0.5, rely=0.5, anchor=tk.CENTER)

        self.btn_eleccion4 = ttk.Button(self.b7, text="Responder a los mensajes con insultos para defenderse", style="Custom2.TButton", command=self.eleccion_mala2)
        self.btn_eleccion4.place(relx=0.5, rely=0.9, anchor=tk.CENTER)

        self.btn_eleccion5 = ttk.Button(self.b7, text="Hablar con sus padres o un adulto de confianza", style="Custom2.TButton", command=self.eleccion_buena2)
        self.btn_eleccion5.place(relx=0.5, rely=0.7, anchor=tk.CENTER)

        self.btn_eleccion6 = ttk.Button(self.b7, text="Hablar directamente con el compañero y preguntar que ocurre", style="Custom2.TButton", command=self.eleccion_buena2)
        self.btn_eleccion6.place(relx=0.5, rely=0.5, anchor=tk.CENTER)

        self.btn_eleccion7 = ttk.Button(self.b11, text="Tomar represalias y contestar agresivamente a los mensajes", style="Custom2.TButton", command=self.eleccion_mala3)
        self.btn_eleccion7.place(relx=0.5, rely=0.9, anchor=tk.CENTER)

        self.btn_eleccion8 = ttk.Button(self.b11, text="Bloquear al usuario, guardar las evidencias y reportar el hecho", style="Custom2.TButton", command=self.eleccion_buena3)
        self.btn_eleccion8.place(relx=0.5, rely=0.7, anchor=tk.CENTER)

        self.btn_eleccion9 = ttk.Button(self.b11, text="Hablar con un adulto de confianza para pedir consejo", style="Custom2.TButton", command=self.eleccion_buena3)
        ...
        self.mensajes = ['''Alex volvió a su casa despues de las vacaciones, animado de haber\nencontrado a una compañera que pensaba igual que el respecto al bulliyng''',
                         '''Despues de hacer los deberes, Alex se dispuso a investigar injusticias que pudieran estar\nocurriendo en las zonas de internet por las que el navegaba''',
                         '''No tardó en encontrar su primera injusticia!!!''']
        self.mensajes2 = ["Navegando por la red social que usaban habitualmente los compañeros de clase, Alex se econtró con que\nuno de sus compañeros de clase hacia algo que no estaba nada correcto",
                          "Estaba hablando mal de otra compañera, burlandose de su forma de hablar y de vestir, y haciendo chistes muy crueles.\nAlex sabia que su compañera era muy buena chica, y no soportaba que se metieran con ella",
                          "Tenia que tomar una decisión, esto no podría continuar!!"]
        self.mensajes3 = ["¿Que debería hacer ALEX?"]
        self.mensajes4 = ["Al principio, Alex pensó que podría ser divertido participar en el acoso, pero\nmuy rptonto se dio cuenta que con eso solo conseguiria hacer mas daño a la víctima, y no era justo"]
        self.mensajes5 = ["Buena elección! Así, Alex puede apoyar y ayudar a su compañera,\ny demostrarle que no esta sola"]
        self.mensajes6 = ['''Un dia Alex recibió unos mensajes muy raros de uno de sus comañeros de clase
Los mensajes eran ofensivos e hirientes y Alex no entendia el motivo\nSiempre se habia llevado muy bien con el y no sabia que podia ocurrir''',
                                  '''Ademas, estos comentarios le estaban haciendo sentirse
muy mal y dolido\nAdemás, era un chico al que veia todos los dias tampoco podia dejarlo pasar''',
                                  '''Tenia que tomar una decisión para hacer frente al problema!!''']
        self.mensajes7 = ["¿Que debería hacer ALEX?"]
        self.mensajes8 = ["Al principio, alex pensó que debería responder con la misma moneda, no era justo el no habia hecho nada!\nSin embargo, pronto se dio cuenta que asi no solucionaria nada, y decidio hablar con el chico.\nEn seguida se dio cuenta que todo habia sido un malentendido y se hicieron amigos de nuevo"]
        self.mensajes9 = ["Muy buena elección! Pagar con la misma moneda no es una buena solución\nEs mucho mejor hablar con un adulto y pedirle consejo o directamente con el propio\ncompañero y tratar de solucionar el problema por uno mismo siempre que se pueda"]
        
        self.mensajes10 = ['''Un dia Alex se encontraba navegando por sus redes sociales favoritas,
cuando de repente se encontró con una situación muy incómoda.
Estaba recibiendo decenas de mensajes privados amenazantes de alguien que no conocia!''',
'''Los mensajes llegaban a diario, y no tenia ni idea de
quien era o de porque la estaba tomando con el.
La situación se volvió muy incómoda, pues a Alex le gustaban las redes sociales y
queria poder seguir utilizandolas''',"Sin embargo, en esa situación no podía continuar usandolas\nTenia que tomar una decisión y hacer frente al problema!"]
        
        self.mensajes11 = ["¿Que debería hacer ALEX?"]
        self.mensajes12 = ["Al principio, Alex pensó en defenderse tomando represalias, pero pronto\nse dió cuenta que eso sólo podía desembocar en un ciclo\nque intensificaría el conflicto, así que en su lugar prefirió\nhablar con un adulto de confianza y pedir ayuda"]
        self.mensajes13 = ["Muy buena elección! El conflicto nunca es la solución\nSiempre hay alternativas mucho mejores para sobrellevar estas situaciones!"]
        self.mensajesfinal = ["FELICIDADES! Has terminado la aventura de Alex!","Si quieres, puedes volver a empezar y elegir a Laura en su lugar\nSeguro que ella tambien tiene interesantes situaciones que solucionar!","Muchas gracias por jugar!!"]
    ...
    def escribir_mensaje(self, i):
        #verifica si i es menor o igual a la longitud del mensaje actual en la lista self.mensajes en la posición self.indice_mensaje_actual. 
        if i <= len(self.mensajes[self.indice_mensaje_actual]):
            self.caja_dialogo.config(text=self.mensajes[self.indice_mensaje_actual][:i])#Si se cumple, se actualiza con los primeros "i" caracters
            self.after(3, lambda: self.escribir_mensaje(i+1))# se utiliza la función after para llamar recursivamente a escribir_mensaje con un incremento en i después de un retardo de 3 milisegundos
        else:
    ...
    def siguiente_mensaje(self):
        ...
        if self.indice_mensaje_actual < len(self.mensajes):
            ...
            self.btn_siguiente.config(text="Continuar", command=self.ir_a_b2)  # habilitar el botón
